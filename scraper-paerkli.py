...
# ═══════════════════════════════════════════════════════════════════════════════
# 1) SPIELPLAN SCRAPING – alle Spiele & Schiri-Dienste der laufenden Saison
# ═══════════════════════════════════════════════════════════════════════════════
...
def scrape_schedule(sched, add_paerkli_games: bool, add_schiri: bool):
    """Spieltage parsen: Pärkli-Spiele und/oder Schiri-Dienste sammeln."""
    if not sched:
        return
    for game_day in sched.select("li.game-day"):
        ...
        if not time_el:
            continue
        date_str = time_el["datetime"]          # ISO-Datum, z.B. "2026-05-17"
        ...
        for row in game_day.select("tbody tr"):
            ...
            # Schiri-Dienste (Pärkli als Schiedsrichter)
...
# Liga 1: Pärkli-Spiele UND Schiri-Dienste erfassen
scrape_schedule(liga1_sched, add_paerkli_games=True,  add_schiri=True)
# Liga 2: nur Schiri-Dienste (Pärkli spielt in Liga 2 nicht mit)
...
print(f"   ✅ Pärkli-Spiele:   {len(spiele)}")
...
# ═══════════════════════════════════════════════════════════════════════════════
# 2) TABELLE via API (alle Liga-Spiele → Tabelle berechnen)
# ═══════════════════════════════════════════════════════════════════════════════
